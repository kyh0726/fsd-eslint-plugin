...
module.exports = {
  meta: {
    ...
    messages: {
      crossLayerImport:
        '레이어 순서 위반: "{{fromLayer}}" 레이어는 "{{toLayer}}" 레이어를 import할 수 없습니다. FSD 레이어 순서를 준수해주세요 (app → pages → widgets → features → entities → shared).',
    },
    schema: [
      {
        type: 'object',
        properties: {
          alias: {
            type: 'string',
            description:
              'Path alias prefix (예: "@" for "@/entities/user"). 기본값: "@"',
          },
          ignorePatterns: {
            type: 'array',
            items: {
            ...
            description: '체크를 무시할 파일 패턴 (정규표현식)',
          },
        },
